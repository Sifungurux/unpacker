...
- Signatures are discovered through the referrers API only — cosign's legacy `sha256-<hex>.sig` tag is never consulted, so a legacy-signed artifact reads as unsigned and fails closed
